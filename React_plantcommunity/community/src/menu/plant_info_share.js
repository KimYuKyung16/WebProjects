...
function Plant_info_share() {
  ...
  let [search_value, setSearchvalue] = useState(); // 검색 단어

  // 검색 기능과 관련된 함수 및 변수들 ---------------------------------------------------------- 

  let [search_total_contents, setSearchTotalcontents] = useState(); // 게시글의 총 개수

  let search_change = (e) => { // 검색값이 바뀔 때마다
  ...
  function search_conents() {
    if (search_value !== '' && search_value !== undefined) {
      ...
      .then(function (response) { // 서버에서 응답이 왔을 때
        ...
        setSearchTotalcontents(count);
        setContents(data);
  
        setTotalpages(parseInt(search_total_contents / one_page_contents));// 총 페이지 개수 설정
        setRemaincontents(search_total_contents % one_page_contents); // 나머지 게시글 개수 설정
  ...
  function each_page_contents(current_page) {
    ...
    if ((search_value === '' || search_value === undefined)) { // 검색모드가 아닐 경우
  ...
  function total_contents_request() { // 게시글의 총 개수
    if (search_value === '' || search_value === undefined) { // 무언가를 검색하지않았을 경우
  ...
  // useEffect(() => { search_conents(); }, [search_value, search_total_contents]) // 검색어가 달라질 때마다
  useEffect(() => { each_page_contents(1); }, [search_value]) // 처음에 무조건 한 번 실행
  useEffect(() => { total_contents_request(); }, [search_value, total_contents, remain_contents]) // 뒤에 변수들의 값이 변할 때마다 실행
  
  useDidMountEffect(() => {
    search_conents();
  }, [search_value, search_total_contents, total_contents, remain_contents]);
